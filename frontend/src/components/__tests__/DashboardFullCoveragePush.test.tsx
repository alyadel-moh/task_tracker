...
vi.mock("../hooks/deleteProjectHook", () => ({
  default: () => ({ mutate: mockDeleteProjectMutate, isPending: false }),
}));
vi.mock("../../hooks/deleteProjectHook", () => ({
  default: () => ({ mutate: mockDeleteProjectMutate, isPending: false }),
}));
...
describe("Dashboard Full Coverage Push Suite", () => {
  ...
  it("switches projects, triggers mobile project selector, and opens modals", async () => {
  ...
  it("triggers task status update mutation and executes success/error callbacks", async () => {
    mockUpdateTaskMutate.mockImplementation((payload, options) => {
      options?.onSuccess?.({ message: "Status updated" });
      options?.onError?.({ response: { data: { message: "Error updating" } } });
    });
